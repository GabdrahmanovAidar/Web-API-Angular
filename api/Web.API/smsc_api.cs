...
public class SMSC
{
    ...
    const string SMSC_LOGIN = "darkx.ru";         // логин клиента
    const string SMSC_PASSWORD = "adminDarkx";    // пароль или MD5-хеш пароля в нижнем регистре
    ...
    const string SMTP_FROM = "support@darkx.ru";     // e-mail адрес отправителя
    const string SMTP_SERVER = "darkx.ru";  // адрес smtp сервера
    const string SMTP_LOGIN = "support@darkx.ru";               // логин для smtp сервера
    const string SMTP_PASSWORD = "support-darkx-Surgut";			// пароль для smtp сервера
    ...
    public string[] send_sms(string phones, string message, int tinyurl = 1,int translit = 0, string time = "", int id = 0, int format = 0, string sender = "", string query = "", string[] files = null)
    {
        string newPhone = "";
        if (phones.StartsWith("8"))
        {
            newPhone = phones.Remove(0, 1);
            phones = newPhone;
        }
        
        if (!phones.StartsWith("+7"))
            phones = "+7" + phones;
        ...
        string[] m = _smsc_send_cmd("send", "cost=3&phones=" + _urlencode(phones)
                        + "&mes=" + _urlencode(message) + "&tinyurl="+tinyurl.ToString() + "&id=" + id.ToString() + "&translit=" + translit.ToString()
...
// string[] r = smsc.send_s ms("79999999999", "http://smsc.ru\nSMSC.RU", 0, "", 0, 0, "", "maxsms=3");
