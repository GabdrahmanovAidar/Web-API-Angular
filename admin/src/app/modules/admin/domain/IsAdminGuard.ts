...
@Injectable()
export class IsAdminGuard implements CanActivateChild, CanActivate, CanLoad {
  ...
  private can(): Observable<boolean> {
    return this.loginService.state$
      ...
      .map((state: LoginState) => state.isLogged && state.type === AccessRoleEnum.SystemAdministrator)
      .do((isAdmin) => {
        if (!isAdmin) {
